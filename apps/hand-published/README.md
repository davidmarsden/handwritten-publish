...
The existing production Vite app remains at the repository root until it can be moved without changing deployment behavior. Hand Published will own:
...
Publisher-specific primitives should live in `packages/publishing-core/` so Hand Published can add destinations without reshaping its document model around any one service.
