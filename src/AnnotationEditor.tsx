...
type Props = {
  ...
  onImportStateChange: (busy: boolean) => void;
...
export default function AnnotationEditor({ page, assets, disabled = false, onChange, onAddAsset, onImportStateChange, onError, onClose }: Props) {
  const [tool, setTool] = useState<Tool>('link');
  const [selectedIndex, setSelectedIndex] = useState<number | null>(null);
  const [dragStart, setDragStart] = useState<Point | null>(null);
  const [dragEnd, setDragEnd] = useState<Point | null>(null);
  const [addingAsset, setAddingAsset] = useState(false);
  const locked = disabled || addingAsset;
  ...
  async function addAsset(file: File) {
    if (!selected || selected.type !== 'photo' || locked) return;
    setAddingAsset(true);
    onImportStateChange(true);
    try {
      const assetId = await onAddAsset(file);
      const current = page.annotations[selectedIndex ?? -1];
      if (selectedIndex !== null && current?.type === 'photo') {
        onChange(replaceAnnotation(page.annotations, selectedIndex, { ...current, assetId }));
      }
    } catch (error) {
      onError(error instanceof Error ? error.message : 'Could not add photo asset.');
    } finally {
  ...
  return (
    <section className="annotationEditor panel" aria-label={`Annotations for ${page.filename}`}>
      <div className="annotationHeader">
        ...
        <button type="button" onClick={onClose} disabled={locked}>Close editor</button>
      </div>

      <div className="annotationToolbar" role="toolbar" aria-label="Annotation tools">
        <button type="button" className={tool === 'link' ? 'active' : ''} onClick={() => setTool('link')} disabled={locked}>Link region</button>
        <button type="button" className={tool === 'photo' ? 'active' : ''} onClick={() => setTool('photo')} disabled={locked}>Photo placeholder</button>
        ...
        <aside className="annotationInspector">
          {selected ? (
            <>
              ...
              {selected.type === 'link' ? (
                <>
                  <label><span>URL</span><input type="url" value={selected.href} placeholder="https://example.com" disabled={locked} onChange={event => updateSelected({ ...selected, href: event.target.value })} /></label>
                  <label><span>Label <em>optional</em></span><input value={selected.label ?? ''} placeholder="What this link is" disabled={locked} onChange={event => updateSelected({ ...selected, label: event.target.value || undefined })} /></label>
                </>
              ) : (
                <>
                  <label>
                    <span>Document photo</span>
                    <select value={selected.assetId} disabled={locked} onChange={event => updateSelected({ ...selected, assetId: event.target.value })}>
                  ...
                  <label className="fileButton assetFileButton">
                    ...
                    <input type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" disabled={locked} onChange={event => { const file = event.target.files?.[0]; if (file) void addAsset(file); event.target.value = ''; }} />
                  </label>
                  ...
                  <label><span>Alt text <em>recommended</em></span><textarea value={selected.alt ?? ''} placeholder="Describe this photo" disabled={locked} onChange={event => updateSelected({ ...selected, alt: event.target.value || undefined })} /></label>
                </>
              )}
              <div className="annotationCoords" aria-label="Normalized region coordinates">
                <label><span>x</span><input type="number" min="0" max="1" step="0.001" value={selected.x} disabled={locked} onChange={event => updateGeometry('x', event.target.value)} /></label>
                <label><span>y</span><input type="number" min="0" max="1" step="0.001" value={selected.y} disabled={locked} onChange={event => updateGeometry('y', event.target.value)} /></label>
                <label><span>w</span><input type="number" min="0.01" max="1" step="0.001" value={selected.width} disabled={locked} onChange={event => updateGeometry('width', event.target.value)} /></label>
                <label><span>h</span><input type="number" min="0.01" max="1" step="0.001" value={selected.height} disabled={locked} onChange={event => updateGeometry('height', event.target.value)} /></label>
              </div>
              <button type="button" className="dangerButton" onClick={removeSelected} disabled={locked}>Delete region</button>
            </>
