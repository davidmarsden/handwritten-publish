...
describe('MicroblogSocialClient', () => {
  it('loads the timeline with paging and bearer auth', async () => {
    ...
    const client = new MicroblogSocialClient({ token: 'abc123', fetchImpl: fetchImpl as typeof fetch });
  ...
  it('routes replies through the explicit reply operation', async () => {
    ...
    expect(JSON.parse(String((init as RequestInit).body))).toEqual({ id: '456', content: 'Hello there' });
  });
  ...
  it('surfaces sanitized bridge errors', async () => {
    const fetchImpl = vi.fn(async () => response({ error: 'Micro.blog request failed.' }, 502));
    const client = new MicroblogSocialClient({ token: 'abc123', fetchImpl: fetchImpl as typeof fetch });
